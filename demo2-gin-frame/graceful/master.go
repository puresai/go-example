	...
	for {
		...
		case err := <-m.workerExit:
			if _, ok := err.(*exec.ExitError); ok {
				fmt.Printf("worker exit with error: %+v, master is going to shutdown.", err)
				m.stop()
				return
			}
			atomic.AddInt32(&m.livingWorkerNum, -1)
			if m.livingWorkerNum <= 0 {
				fmt.Printf("all workers exit, master is going to shutdown.")
				m.stop()
				return
			}
			continue
		case sig = <-ch:
			fmt.Printf("master got signal: %v\n", sig)
		}
...
func (m *master) reload() {
	...
	p, err := m.forkWorker()
	if err != nil {
		fmt.Printf("[reload] fork worker error: %v\n", err)
		return
	}
